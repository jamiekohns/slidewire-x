...
it('resolves effective settings with slide > deck > config precedence', function (): void {
    ...
    $slides = [
        new Slide(id: 'test-0', html: '', meta: ['theme' => 'white', 'transition' => 'zoom'], h: 0, v: 0),
...
it('falls back to slide config when deck and slide have no overrides', function (): void {
    ...
    $slides = [
        new Slide(id: 'test-0', html: '', meta: [], h: 0, v: 0),
    ];
...
it('preserves original slide data alongside effective settings', function (): void {
    ...
    $slides = [
        new Slide(id: 'test-0', html: '<h1>Hello</h1>', meta: ['notes' => 'Test notes'], fragments: 2, class: 'bg-red', h: 0, v: 0),
    ];
    ...
    expect($result[0]->id)->toBe('test-0')
        ...
        ->and($result[0]->effective)->not->toBe([]);
});
